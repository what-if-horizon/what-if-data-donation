...
# Select the Json name without the .json
...
# Save the folder name of the folder in which the json is stored
id_df['folder1'] = id_df['file_path'].str.strip('/').str.split('/').str[-2]
# Save the folder name of the folder one level higher
...
for n in range(1,4):
    ...
    for i, row in duplicates.iterrows():
        ...
        if pd.notna(path):
            ...
            # Select the part of the path from lower to higher in each iteration
            path_rest = path[-n:]
            ...
            # Combine the lowest folder name, the json and the list containing part of the path 
            # ... necessary to make the ID unique
            id_list = [folder1] + [name] + path_rest

            # Join all items in the list together
            new_id = ':'.join(id_list)
...
# If the steps above have not been sufficient to create unique IDs, we go one step further 
# ... and add the folder in which the folder containing the json is stored
...
merged_df.to_csv(f"{final_directory}/IG_Merged_structures.csv", index=False)

time = datetime.datetime.now()
print(f'{time}: IG_Merged_structures.csv saved')
