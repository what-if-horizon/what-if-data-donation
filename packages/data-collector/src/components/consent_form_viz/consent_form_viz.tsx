...
export const ConsentFormViz = (props: Props): JSX.Element => {
  ...
  const { description, donateQuestion, donateButton, cancelButton } =
    prepareCopy(props);
  ...
  const updateTable = useCallback(
    (tableId: string, table: TableWithContext) => {
      setTables((tables) => {
  ...
  function parseTables(
    tablesData: PropsUIPromptConsentFormTableViz[]
  ): Array<PropsUITable & TableContext> {
    return tablesData.map((table) => parseTable(table));
  }

  function parseTable(
    tableData: PropsUIPromptConsentFormTableViz
...
function prepareCopy({
  ...
  return {
    ...
    donateQuestion: Translator.translate(
      donateQuestion ?? defaultDonateQuestionLabel,
      locale
    ),
    donateButton: Translator.translate(
      donateButton ?? defaultDonateButtonLabel,
      locale
    ),
...
const defaultDescription = new TextBundle()
  .add(
    "en",
    "Determine whether you would like to share the data below. Carefully check the data and adjust when required. With your contribution, you help the previously described research. Thank you in advance."
  )
  .add(
    "de",
    "Legen Sie fest, ob Sie die untenstehenden Daten teilen möchten. Überprüfen Sie die Daten sorgfältig und passen Sie sie bei Bedarf an. Mit Ihrem Beitrag helfen Sie der zuvor beschriebenen Forschung. Vielen Dank im Voraus."
  )
  .add(
    "nl",
    "Bepaal of u de onderstaande gegevens wilt delen. Bekijk de gegevens zorgvuldig en pas zo nodig aan. Met uw bijdrage helpt u het eerder beschreven onderzoek. Alvast hartelijk dank."
  )
  .add(
    "es",
    "Determine si desea compartir los datos a continuación. Revise cuidadosamente los datos y ajústelos cuando sea necesario. Con su contribución, ayuda a la investigación previamente descrita. Muchas gracias de antemano."
  );
